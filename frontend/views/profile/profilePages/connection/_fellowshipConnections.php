...
if ($indvArray) {
	foreach ($indvArray as $indv) {
		echo '<div class="connection-container">';
			echo '<div class="connection">';
				echo (empty($indv->image2) ? Html::img('@web/images/content/profile-logo.png'): Html::img($indv->image2)); 
				echo '<div class="title">';
					echo Html::a($indv->getFormattedNames()->formattedNames . '&nbsp' . Html::icon('link', ['class' => 'internal-link']), [ProfileController::$profilePageArray[$indv->type], 'urlLoc' => $indv->url_loc, 'name' => $indv->url_name, 'id' => $indv->id]);
...
if (empty($staffArray) && empty($indvArray) && empty($churchArray) && empty($likeArray)) {
